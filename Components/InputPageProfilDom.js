...
function InputPageProfilHome(props) {
   ...
   var onChangeDestination = async (destination) => {
    ...
    var handleSaveAddress = () => {
        fetch(`http://${IpAdress}:3000/addHomeAddress?homeaddress=${homeAddress}&userid=${props.id}`)
        .then(response => {
        ...
        .then(data => {
            ...
            if(data.response) {
                props.saveAdress(homeAddress);
                let newHomeAddress = {homeaddress: homeAddress}
                AsyncStorage.mergeItem('userVTC', JSON.stringify(newHomeAddress));
    ...
    const predictionsAdress = predictions.map(predictions => (
        <Text style={styles.predictionsStyle} onPress={() => {setHomeAddress(predictions.description), setPredictions([])}} key={predictions.id}>  {predictions.description} </Text>
      ))
...
function mapDispatchToProps(dispatch) {
    return {
        saveAdress: function(homeaddress) {
            dispatch(
                {
                    type: 'saveHomeAddress',
